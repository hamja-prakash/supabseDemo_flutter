        ...
        appBar: AppBar(
          ...
          actions: [
            PopupMenuButton(
              ...
              itemBuilder: (context) => [PopupMenuItem(onTap: logout, child: const Text(AppConstants.logout))],
            ),
          ],
        ),
                ...
                builder: (context, state) {
                  if (state is NoteLoading) {
                    ...
                    if (state.notes.isEmpty) {
                      return const Center(
                        child: Text(AppConstants.noNotesFound, style: TextStyle(fontWeight: .normal, fontSize: 20)),
                      );
                    }
                    return SafeArea(
                      bottom: true,
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return Card(
                            ...
                            child: ListTile(
                              contentPadding: const EdgeInsets.only(right: 4, left: 16, top: 4, bottom: 4),
                              title: Text(note[AppConstants.titleKey] ?? ''),
                              subtitle: Text(note[AppConstants.descriptionKey] ?? ''),
                              leading: Image.asset(AssetPath.bookLogo, color: Colors.deepPurple, width: 24, height: 24),
                              trailing: Row(
            ...
            return FloatingActionButton(
              onPressed: () {
                ...
                Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (context) => BlocProvider.value(value: cubit, child: const AddNoteScreen()),
                  ),
                );
              },
